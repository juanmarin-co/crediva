...
```bash
uv run lending-interest-rates convert
```

Use `--raw-path` and `--parquet-path` to override the default `data/raw` and `data/parquet` locations. Conversion uses one DuckDB external-execution query for all changed months and reads each candidate raw page once. DuckDB automatically uses all processors available to the process, limits managed memory to 8 GB, and spills larger sorts to temporary disk under the Parquet directory. Structured `READ`, `WRITE`, and `DELETE` events report every data or manifest file used or changed, making incremental work observable.
