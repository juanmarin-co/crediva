...
The build prerenders every static route to `apps/dash/dist/client` and also produces a Cloudflare Worker bundle in `apps/dash/dist/server`. Dynamic parameter routes must be given explicit paths or reachable links to be prerendered; check the build log whenever adding routes. For a local Cloudflare Workers deployment, authenticate with Wrangler, then run `pnpm --filter @crediva/dash deploy`. For GitHub deployments, set the `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repository secrets. The **Deploy dashboard** workflow starts on every push to `main` and can also be started manually from `main`. New runs cancel older in-progress runs rather than queue them. Use an API token scoped to the deployment account with Edit Cloudflare Workers permission. No deploy is required for local development.
